...
smoke() {
  ...
  # fell back to allow{}.
  for svc_pair in "browser:7984:bb-browser" "scheduler:7982:bb-scheduler-admin"; do
...
summary() {
  ...
  Public (TLS via Let's Encrypt cert for $PUBLIC_HOSTNAME):
    ...
    Dex (OIDC IdP)   : https://$PUBLIC_HOSTNAME:5556/.well-known/openid-configuration
                       (browser & scheduler-admin redirect here for GitHub OAuth → team check)
