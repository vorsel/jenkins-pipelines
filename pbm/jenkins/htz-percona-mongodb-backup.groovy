library changelog: false, identifier: 'lib@ENG-7_hetzner_pbm_test', retriever: modernSCM([
    $class: 'GitSCMSource',
...
pipeline {
    ...
    stages {
        stage('Create PBM source tarball') {
            ...
            steps {
                slackNotify("@alex.miroshnychenko", "#00FF00", "[${JOB_NAME}]: starting build for ${GIT_BRANCH} - [${BUILD_URL}]")
                cleanUpWS()
    ...
    post {
        ...
        failure {
            slackNotify("@alex.miroshnychenko", "#FF0000", "[${JOB_NAME}]: build failed for ${GIT_BRANCH} - [${BUILD_URL}]")
            deleteDir()
        }
        always {
            sh '''
