...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        ...
        if [ -f test/percona-server-mongodb-60.properties ]; then
            cp test/percona-server-mongodb-60.properties percona-server-mongodb-60.properties.backup
        fi
        rm -rf test/*
        mkdir -p test
        # Restore properties file if it was backed up
        if [ -f percona-server-mongodb-60.properties.backup ]; then
            mv percona-server-mongodb-60.properties.backup test/percona-server-mongodb-60.properties
        fi
    ...
    stages {
        stage('Create PSMDB source tarball') {
            ...
            steps {
                ...
                stash includes: 'test/percona-server-mongodb-60.properties', name: 'psmdb-properties'
        ...
        stage('Push to public repository') {
            ...
            steps {
                ...
                script {
                    ...
                    sync2PrivateProdAutoBuild(params.CLOUD, PSMDB_REPO, COMPONENT)
                }
            }
        }
